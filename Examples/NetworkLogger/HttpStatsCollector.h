	...
	void collectHttpGeneralStats(pcpp::TcpLayer* tcpLayer, pcpp::HttpMessage* message, uint32_t flowKey)
	{
		...
		if (message->getProtocol() == pcpp::HTTPRequest)
		{
			...
			if (m_FlowTable[flowKey].curSeqNumberRequests >= ntohl(tcpLayer->getTcpHeader()->sequenceNumber))
				return;
		    pcpp::HttpRequestLayer* req = ((pcpp::HttpRequestLayer*)message);
            m_FlowTable[flowKey].url = strdup(req->getUrl().c_str());
//            m_FlowTable[flowKey].url.erase(std::find(m_FlowTable[flowKey].url.begin(), m_FlowTable[flowKey].url.end(), '\0'), m_FlowTable[flowKey].url.end());
            printf("Deneme %s",  m_FlowTable[flowKey].url);
//            try{
//                std::cout << "Deneme " <<   m_FlowTable[flowKey].url;
//            }catch (int e){
//            }
//            fflush( stdout );
	...
	void collectRequestStats(pcpp::HttpRequestLayer* req)
	{
		...
		std::string hostFieldValue = NULL;
		if (hostField != NULL){
		    hostFieldValue = hostField->getFieldValue();
			m_RequestStats.hostnameCount[hostFieldValue]++;
        }
